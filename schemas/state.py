...
class CreationState(TypedDict):
    ...
    # 结构：{basic_rules, power_structure, geography, world_taboos, unique_settings}
    world_setting: dict
    ...
    # 用户确认后的人物卡（human_review_write 写入，bible_update 保存到 entity_cards）
    confirmed_char_cards: list[dict]
    ...
    pending_noun_foreshadows: list[dict]       # [{surface_meaning, mentioned_by, story_potential}]
    noun_foreshadow_story_potential_edits: dict[str, str]  # {surface_meaning: story_potential}
    ...
    auto_retry_count: int       # 兼容旧版，等价 auto_retry_count_path
    auto_retry_count_path: int   # path 类型重试次数
    auto_retry_count_expand: int  # expand 类型重试次数
    auto_retry_count_write: int   # write 类型重试次数
    auto_total_retry_count: int  # 当前章节总重试次数（跨类型），章节完成后清零
    pending_review_type: str     # 当前待审核的类型：'path'/'expand'/'write'/'batch'

    # 张力检查（write 与 bible_update 之间）
    tension_retry_count: int     # 张力检查重试次数，通过后重置为 0
    tension_minor_issues: list   # 通过但存在的轻微张力不足（供 human_review 展示参考）
    ...
    # ── v4.3 事件驱动分层叙事架构字段 ────────────────────────────────────────
    # 锚点推进状态，由 bible_update 维护
    # 结构：{completed: [anchor_id, …], pending: [anchor_id, …]}
    anchor_progress: dict

    # 路径推进状态，由 bible_update 维护
    # 结构：{current_event_id, completed_paths: [path_id, …], remaining_paths: [path_id, …]}
    path_progress: dict

    # 已完成事件摘要列表，由 event_chain_gen 逐事件追加，用于全局防重复
    # 每项：{event_id, event_name, conflict_type, primary_resource_used, anchor_touched}
    completed_events_summary: Annotated[list, lambda x, y: y]
    ...
    #        collision{}, event_core{}, causal_chain{}, anchor_check{}, global_context_check{}
    current_event: dict

    # 当前事件拆解出的路径列表（path_gen 写入，expand1 逐路径消费）
    # 结构：{event_id, event_name, total_paths, paths[], rhythm_note, global_rhythm_adjustment}
    current_event_paths: dict

    # narrative_extract 节点输出（在正文通过审核后触发）
    # 结构：{foreshadows: [...], chapter_path_chain: {...}, lexicon_updates: [...], global_seq: int}
    narrative_extract_result: dict
