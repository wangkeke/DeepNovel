...
每个 path 对应一次后续 write 的正文节拍；**具体字数上下限以写作阶段用户消息中的「字数区间铁律」为准**。规划单路径体量时以 **约 1500 字内能写完整** 为硬参考（与写作铁律一致）；估算密度可按 **1000～1500 字/路径** 为主区间（高张力可偏上限，过渡/信息可偏下限），须与后续注入的字数铁律不矛盾。

## 编剧视角铁律
**你的问题不是「角色做了什么」，而是「读者需要经历哪几个时刻」。**

### 推导两步法（叙事单元 → 字数校验）
**第一步：清点叙事单元（只计「有独立戏剧增量」的拍）**  
下列维度用于自检「本事件可能需要哪些关键时刻」，**不是勾选几项就必须输出几条路径**；同一连贯场面内应合并；开门、赶路、掏钥匙等碎动作不得单独成径。
- 场面/空间是否发生**质变**（若仅为位移而无对峙或信息质变，并入上一拍）
- **关键决策**（影响后续走向的选择）
- **信息揭示**（读者或角色获得关键认知跃迁）
- **情绪或态度转折**
- **对抗高潮**（博弈、冲突、追逃等核心交锋）
...
**第二步：单路径字数校验**  
每个叙事单元必须能在 **约 1500 字内写完整**（与写作铁律一致）。预计超载则拆成两条路径；若多个单元可在同一场面写完则合并为一条。

**上游边界**：若严格推导后仍感到「客观需要远超 8 条路径」才能完成本事件，须在输出的 `narrative_moments_reasoning` 中写明：**理想情况下该 macro 事件应在 event_chain_gen 阶段拆成两个独立事件**。当前任务仍须遵守下文 **paths 至多 8 条**，通过合并次要单元满足硬顶。

**标签怎么用**：两步法定下「有几拍」之后，再为每拍填写 `narrative_function`（备选：`建立信息` / `制造悬念` / `释放张力` / `人物呈现` / `埋下伏笔` / `喘息`）。禁止先套功能模板再往里面填剧情。

**禁止反向操作**：禁止预设「需要一个喘息」「需要一个释放张力」然后往里填内容——那是套模板，不是编剧。

### 路径数量原则（动态条数 + Compaction）
- 以事件实际叙事体量决定路径数；**禁止默认固定条数**（尤其禁止无意识地默认 4 条）。
- 参考分档（网文节奏，非八股）：简单约 **2** 条；标准约 **3～4** 条；复杂约 **5～6** 条；仅当事件定义明确为大型高潮/终局对峙时可接近 **8** 条。
- **禁止**把开门、走路、掏钱等无戏剧增量的碎动作拆成独立路径，须并入相邻叙事时刻。
- **超过 6 条须自检**：是否过度贪婪拆解；能合并则合并。**输出的 paths 数组长度不得超过 8**（绝对上限）。
- **禁止**为了凑数而拆分无实质区别的路径。路径条数直接影响后续 write 调用次数与费用，宁合并勿碎拆。

### 节奏自检（输出后自查）
- 结合 user 中的 **recent_rhythm**：若近期路径连续高张力，本事件宜安排喘息拍；若连续铺垫，宜有释放张力拍，避免单调。
...
## 任务
**第一步**：用「叙事单元自检」（场面质变、决策、揭示、转折、对抗、收束）列出读者必须亲历的时刻；只保留**有戏剧增量**的拍，剔除碎动作。
**第二步**：对每个单元做 **1500 字内能否写完整** 的校验——超载则拆路径，可合并则并径。
**第三步**：对照路径数量原则定稿条数（通常 **2～6** 条；必要时至多 **8** 条；禁止默认 4；超过 6 条须合并碎径）。若仍感到事件体量过大，在 `narrative_moments_reasoning` 注明「建议上游拆事件」，输出仍须 ≤8 条。
**第四步**：为每个时刻填写完整路径定义，再由内容推导 `narrative_function` 标签。

返回 JSON（字段顺序固定）：
{{
  "event_id": "{event_id}",
  "event_name": "{event_name}",
  "narrative_moments_reasoning": "（CoT）叙事单元推导、1500字校验、最终条数理由；若事件体量过大可注明建议 event_chain 拆分（输出路径仍≤8）",
