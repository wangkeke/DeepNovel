...
每个 path 对应一次后续 write 的正文节拍；**具体字数上下限以写作阶段用户消息中的「字数区间铁律」为准**。规划路径体量时可按约 **1000～1500 字/路径** 量级把握事件密度（高张力可偏上限，过渡/信息可偏下限）。
...
**禁止反向操作**：禁止预设「需要一个喘息」然后往里填内容——那是套模板，不是编剧。

### 路径数量原则（动态拆解与防爆锁）
- 以事件的实际叙事体量决定路径数，绝对范围：**2～8 条**。
- **简单事件**（如情报交换、日常对话）：2 条路径
- **标准事件**（如一次冲突、一场交锋）：3-4 条路径
- **复杂事件**（如多方博弈、连环反转）：5-6 条路径
- **史诗事件**（如卷末终极对决）：7-8 条路径
- 🚨 **强制反思阈值**：如果你的拆解结果达到或超过 6 条路径，必须立刻自我反省——是不是把过多的细枝末节（如"开门"、"走路"、"掏钱"）拆成了独立时刻？请强制将那些琐碎的中间动作合并到主要的叙事时刻中！绝对禁止超过 8 条！
...
### 与 event_chain 输出的对齐铁律
1. **交汇类型 (`intersect_type`) 对齐**：
   - 【错位擦肩】：路径须体现未照面、信息差、迟到的发现或误读。
   - 【单边推进】：须明确本事件以哪一线为主轴，另一线如何以缺席/后果渗透。
   - 【正面碰撞】：允许强对峙与当场交锋。禁止无视该字段一律写成当面决战。
2. **主角能动性与结果类型 (`protagonist_tick_type` & `event_result_type`) 对齐**：
   - **A/B类主动 Tick**：路径应呈现「主角在营生或追线中主动撞上意外」，避免章章都是被动接招。
   - **A_closed (闭合型)**：允许更多日常收束与阅历余韵路径。
   - **B_fragment (碎片型)**：须保留至少一条路径让读者感到「这事没这么简单」，种下主线疑点。
   - **C_open (开放型)**：最后一条路径落下开放压力或新日程，切勿打成闭环胜仗。
...
**第二步**：根据时刻内容判断路径数量（2～8个，视实际体量；禁止默认4个）。
**第三步**：为每个时刻填写完整路径定义。
...
{{
  ...
  "paths":[
    {{
      ...
      "path_name": "路径标题（10-30字）",
      "moment_description": "【核心铁律：Show, Don't Tell】读者此刻看到了什么/感受到了什么张力？必须用环境、心理、压力差来定义这一刻！绝对禁止写成‘主角去干嘛’的动作流水账！",
      "narrative_function": "由 moment_description 推导出的功能标签（限上述6种备选之一）",
      "pov_character": "本路径的视角角色（主角 或 其他角色名）",
      "key_characters_present": ["出场人物列表"],
      "scene_anchor": "本路径发生的具体物理场景/地点（1-3句描述）",
      "scene_exit": "【物理边界刹车】本路径正文结束时的具体画面/物理状态（人站在哪、看向哪、做完了什么动作）。这是下游 Write 节点的绝对停止点，必须极其精确！",
