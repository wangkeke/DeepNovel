...
async def narrative_extract_node(
    ...
    deviation_label = "⚠️ 有偏差" if deviation else "✅ 路径一致"
    ...
    _display_narrative_extract_result(
        foreshadows, path_chain_info, new_count, update_count, lexicon_updates
    )
...
def _display_narrative_extract_result(
    ...
    if deviation:
        chain_content += f"[yellow]⚠️  路径偏差：{deviation_note}[/yellow]"
    else:
        chain_content += "[green]✅ 路径执行一致[/green]"
